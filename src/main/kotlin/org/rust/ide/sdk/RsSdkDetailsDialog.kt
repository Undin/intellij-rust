...
import org.rust.ide.sdk.RsSdkUpdater.Companion.updateLocalSdkVersion
...
class RsSdkDetailsDialog(
    ...
    private fun editSdk() {
        ...
        if (dialog.showAndGet()) {
            sdkList.repaint()
            val needsReload = currentSdk.homePath != dialog.toolchainPath || modificator.sdkAdditionalData != dialog.data
            if (needsReload || modificator.name != dialog.name) {
                modifiedModificators.add(modificator)
                modificator.name = dialog.name
                modificator.homePath = dialog.toolchainPath
    ...
    private fun reloadSdk(currentSdk: Sdk) {
        updateLocalSdkVersion(currentSdk, modificators[currentSdk])
        modifiedModificators.remove(modificators[currentSdk])
        modificators[currentSdk] = currentSdk.sdkModificator
    }
